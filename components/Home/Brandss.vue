<template>
  <div v-if="brands" class="w-full">
    <div class="flex flex-col py-6 bg-white lg:py-12 md:flex-row">
      <div
        class="w-full my-auto text-lg antialiased font-semibold tracking-widest text-center transform lg:py-6 md:-rotate-90 text-secondary-500 lg:w-1/6 md:w-1/4"
      >
        Brands
      </div>
      <div
        class="grid w-full grid-cols-2 mx-auto sm:grid-cols-3 lg:grid-cols-5 lg:w-5/6 md:w-3/4"
      >
        <div v-for="b in brands.data" :key="b.id" class="mx-auto">
          <nuxt-link :to="localePath('/c')" class="p-1">
