<template>
  <div class="flex flex-col h-auto py-6 pt-10 bg-white">
    <div class="flex justify-center mx-5 border-b">
      <span
        class="pb-2 mt-2 text-xl font-normal text-center sm:text-2xl lg:text-3xl border-b4 text-secondary-500 border-secondary-500 md:mt-4"
      >
        Categories
      </span>
    </div>
    ...
    <div
      v-if="categories"
      class="grid grid-cols-3 gap-4 p-1 px-3 md:grid-cols-4 lg:grid-cols-6 lg:px-28 md:p-8 md:px-24"
    >
      <div
      ...
      >
        <nuxt-link :to="localePath(`/c/${c.slug}`)">
          ...
          />
          <span
            class="flex justify-center mx-auto mt-4 text-base font-normal text-center text-gray-500"
          >
            {{ c.name }}
          </span>
        </nuxt-link>
