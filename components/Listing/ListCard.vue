<template>
  <div
    class="z-0 w-full hover:shadow md:p-1 focus:outline-none full-height rounded-2xl"
  >
    <div class="flex flex-row-reverse w-full bg-white focus:outline-none">
      <div
        v-if="product"
        class="relative items-center w-full mx-auto bg-white rounded-lg totalheight focus:outline-none"
      >
        <div
          class="relative flex flex-row justify-between px-2 pt-2 mx-auto my-auto bg-red-300 md:relative"
        >
          <div
            class="relative flex flex-row bg-white border cursor-pointer border-primary-500 trans"
          >
            <!-- bag icon  -->
            <svg
          ...
          <div class="my-auto">
            <button
              ...
              class="relative z-10 bg-white border border-gray-500 rounded-full hover:border-gray-400 similar focus:outline-none"
            >
              <svg
                class="w-5 h-5 text-gray-500 fill-current hover:text-gray-400 heart"
                v-bind:class="[
                  isActive
        ...
        <div class="cart">
          <nuxt-link :to="localePath(`/${product.slug}?id=${product.id}`)">
            <span
              class="flex flex-row items-center h-40 mx-auto overflow-hidden md:w-full md:h-56"
            >
              <img
                v-lazy="product.img || (product.images && product.images[0])"
                alt=""
                class="object-cover mx-auto overflow-hidden md:pt-0 md:p-0 md:object-contain"
              />
            </span>
          </nuxt-link>
        </div>
        <div
          class="relative bottom-0 w-full pb-4 overflow-hidden md:absolute md:pb-0 nameall"
        >
          <nuxt-link :to="localePath(`/${product.slug}?id=${product.id}`)">
            <div class="p-2 mx-auto text-sm text-center rounded-b-lg">
              <div>
                ...
                <div
                  class="overflow-hidden text-gray-700 truncate md:text-lg text-normal"
                >
                  {{ product.name }}
                </div>
              </div>
              <div class="flex justify-center mx-auto mt-2">
                <div class="flex flex-row conatiner text">
                  <div class="font-semibold text-gray-600 me-1 md:me-2">
                    {{ product.price | currency(settings.currency_symbol) }}
                  </div>
                  <strike
                    class="my-auto text-xs font-normal text-gray-500 me-1 md:me-2"
                    >{{
          ...
          <div class="h-12 mt-1 overflow-hidden md:hidden">
            <div class="container absolute flex flex-col items-center mx-auto">
              ...
              >
                <span
                  v-if="addedToCart"
                  class="w-full px-3 py-1 text-sm font-semibold text-white uppercase bg-gray-400 rounded-2xl focus:outline-none"
                >
                  Added
                </span>
                <span
                  v-else
                  class="flex flex-row w-full px-3 py-1 text-sm font-semibold text-white uppercase transition duration-300 ease-in-out transform bg-secondary-500 rounded-2xl focus:outline-none hover:opacity-75 hover:-translate-y-1 hover:scale-100"
                >
                  <svg
