<template>
  <div>
    ...
    <MobileFilters
      class="sticky top-0 z-20 flex-none mt-0 sm:hidden"
      :count="productCount"
    ...
    <div class="container flex mx-auto sm:mt-6 xl:mt-0">
      <DesktopFilters
        class="sticky top-0 flex-none hidden max-w-xs min-h-screen mt-5 bg-gray-100 shadow ms-4 lg:block"
        :facets="facets"
        :fl="fl"
      ...
      <div class="relative w-full px-4">
        ...
        <div v-else class="sm:mt-0">
          <div
            v-if="loading"
            class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5"
          >
            <ProductSkeleton v-for="(p, ix) in 10" :key="ix + '-1'" />
          </div>
          <div
            v-else-if="products && products.length > 0"
            class="flex flex-col flex-shrink-0 w-full h-full mt-4 sm:mt-2 nowrap flex-nowrap"
          >
            <div
              class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5"
            >
