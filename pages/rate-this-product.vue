      ...
      <div class="flex flex-col justify-between md:flex-row">
        ...
        <div v-if="product" class="flex flex-row me-5">
          <div class="p-2 pl-4 my-auto text-gray-700">
            ...
            <!-- <span class="flex justify-end text-xs text-secondary-200 text-end">{{product.price | currency(settings.currency_symbol)}}</span>-->
          </div>
          <img
            v-lazy="product.img"
            alt="product img"
            class="object-contain w-16 h-16 m-2 bg-gray-200 shadow ms-4 border-1"
          />
        </div>
      </div>
    </div>
            ...
            >
              ...
              <span>
                <textarea
                  ...
                  class="w-full h-40 p-3 text-sm border-none rounded-sm shadow ring-2 ring-gray-400 focus:ring-2 focus:ring-accent-700"
                ></textarea>
              </span>
              <button
                type="submit"
                class="float-right w-1/4 h-10 p-1 mt-4 text-white bg-primary-500 rounded shadow lg:text-xl md:text-lg focus:outline-none md:h-12 hover:-translate-y-0.5 transition transform ease-in-out focus:ring-opacity-50 focus:ring focus:ring-offset-2 focus:ring-yellow-600 hover:opacity-80"
              >
                SUBMIT
              </button>
            </form>
...
import { StarRating, Ratingcircle } from '~/shared/components/ui'
...
export default {
  ...
  middleware: ['isAuth'],
  components: { StarRating, Ratingcircle, Nav },
  data() {
    return {
      product: null,
      saveReview: {
